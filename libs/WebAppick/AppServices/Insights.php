...
class Insights {
	...
	public function __construct( $client, $name = null, $file = null ) {
		...
		if ( is_object( $client ) && is_a( $client, 'Challan\AppServices\Client' ) ) {
			$this->client = $client;
		}
	}
	...
	protected function data_we_collect() {
		$data = array(
			esc_html__( 'Server environment details (php, mysql, server, WordPress versions).', 'webappick-pdf-invoice-for-woocommerce' ),
		);
	...
	public function admin_notice() {
		...
		if ( ! $this->__is_local_server() ) {
			...
			if ( empty( $this->notice ) ) {
				$notice = sprintf(
					apply_filters(
						...
						esc_html__(
							'Want to help make %1$s even more awesome? Allow %1$s to collect non-sensitive diagnostic data and usage information.',
			...
			echo '&nbsp;<a href="' . esc_url( $this->get_opt_out_url() ) . '" class="button button-secondary">' . esc_html__(
				'No thanks',
				'webappick-pdf-invoice-for-woocommerce'
			) . '</a>';
	...
	public function handle_optIn_optOut() {
		...
		if ( isset( $_GET[ $this->client->getSlug() . '_tracker_optOut' ] ) && $_GET[ $this->client->getSlug() . '_tracker_optOut' ] == 'true' ) {
			$this->optOut();
			wp_redirect( remove_query_arg( $this->client->getSlug() . '_tracker_optOut' ) );
			exit;
		}
	}
	...
	public function add_removable_query_args( $removable_query_args ) {
		return array_merge(
			...
			array( $this->client->getSlug() . '_tracker_optIn', $this->client->getSlug() . '_tracker_optOut' )
		);
	}
	...
	public function add_weekly_schedule( $schedules ) {
		$schedules['weekly'] = array(
			...
			'display'  => __( 'Once Weekly', 'webappick-pdf-invoice-for-woocommerce' ),
		);
	...
	private function __get_uninstall_reasons() {
		...
		$reasons = array(
			array(
				...
				'text'        => esc_html__( 'I couldn\'t understand how to make it work', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Would you like us to assist you?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
			array(
				'id'          => 'found-better-plugin',
				'text'        => esc_html__( 'I found a better plugin', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => 'text',
				'placeholder' => esc_html__( 'Which plugin?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
			array(
				...
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us more about that feature?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
			array(
				'id'          => 'is-not-working',
				'text'        => esc_html__( 'The plugin is not working', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us a bit more whats not working?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
			array(
				'id'          => 'looking-for-other',
				'text'        => esc_html__( 'It\'s not what I was looking for', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => '',
				'placeholder' => '',
			),
			array(
				'id'          => 'did-not-work-as-expected',
				'text'        => esc_html__( 'The plugin didn\'t work as expected', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'What did you expect?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
			array(
				'id'          => 'debugging',
				'text'        => esc_html__( 'Temporary deactivation for debugging', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => '',
				'placeholder' => '',
			),
			array(
				'id'          => 'other',
				'text'        => esc_html__( 'Other', 'webappick-pdf-invoice-for-woocommerce' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us a bit more?', 'webappick-pdf-invoice-for-woocommerce' ),
			),
		);
	...
	public function uninstall_reason_submission() {
		...
		if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), $this->client->getSlug() . '_insight_action' ) ) {
	...
	public function support_ticket_submission() {
		...
		if ( empty( $this->ticketTemplate ) || empty( $this->ticketRecipient ) || empty( $this->supportURL ) ) {
			wp_send_json_error( sprintf( '<p class="mui-error">%s<br>%s</p>', esc_html__( 'Something Went Wrong.', 'webappick-pdf-invoice-for-woocommerce' ), esc_html__( 'Please try again after sometime.', 'webappick-pdf-invoice-for-woocommerce' ) ) );
			wp_die( -1 );
		}
		if (
			...
			if ( wp_mail( $this->ticketRecipient, sanitize_text_field( $_REQUEST['subject'] ), sprintf( '<div>%s</div>', $this->ticketTemplate ), $headers ) ) {
				...
				if ( false !== ( $supportResponse = apply_filters( "WebAppick_{$projectSlug}_Support_Request_Ajax_Success_Response", false, $_REQUEST ) ) ) {
					...
					$this->supportResponse = sprintf(
						'<h3>%s</h3>',
						esc_html__( 'Thank you -- Support Ticket Submitted.', 'webappick-pdf-invoice-for-woocommerce' )
					);
				}
				...
				if ( false !== ( $supportErrorResponse = apply_filters( "WebAppick_{$projectSlug}_Support_Request_Ajax_Error_Response", false, $_REQUEST ) ) ) {
					...
					$this->supportErrorResponse = sprintf(
						...
						esc_html__( 'Something Went Wrong. Please Try Again After Sometime.', 'webappick-pdf-invoice-for-woocommerce' )
					);
				}
				wp_send_json_error( $this->supportErrorResponse );
			}
		} else {
			wp_send_json_error( sprintf( '<p class="mui-error">%s</p>', esc_html__( 'Missing Required Fields.', 'webappick-pdf-invoice-for-woocommerce' ) ) );
		}
		die();
	}
		...
		<div class="wapk-dr-modal"
			...
			<div class="wapk-dr-modal-wrap support" style="display: none;">
				<div class="wapk-dr-modal-header">
					<h3><?php esc_html_e( 'Submit Support Ticket.', 'webappick-pdf-invoice-for-woocommerce' ); ?></h3>
					<a href="javascript:void 0;" class="wapk-dr-modal-close" aria-label="<?php esc_attr_e( 'Close', 'webappick-pdf-invoice-for-woocommerce' ); ?>">
				...
				<div class="wapk-dr-modal-body">
					...
					<div class="wapk-row mui col-2 col-right">
						<label for="wapk-support-email" class="shrink"><?php esc_html_e( 'Email', 'webappick-pdf-invoice-for-woocommerce' ); ?></label>
						<div class="wapk-form-control">
					...
					<div class="clear"></div>
					<div class="wapk-row mui col-2 col-left">
						<label for="wapk-support-subject"><?php esc_html_e( 'Subject', 'webappick-pdf-invoice-for-woocommerce' ); ?></label>
						<div class="wapk-form-control">
							<input type="text" name="subject" id="wapk-support-subject" required>
						</div>
					</div>
					<div class="wapk-row mui col-2 col-right">
						<label for="wapk-support-website" class="shrink"><?php esc_html_e( 'Website', 'webappick-pdf-invoice-for-woocommerce' ); ?></label>
						<div class="wapk-form-control">
					...
					<div class="wapk-row mui">
						<label for="wapk-support-message"><?php esc_html_e( 'Message', 'webappick-pdf-invoice-for-woocommerce' ); ?></label>
						<div class="wapk-form-control">
				...
				<div class="wapk-dr-modal-footer">
					<button class="button button-primary send-ticket"><?php esc_html_e( 'Send Message', 'webappick-pdf-invoice-for-woocommerce' ); ?></button>
					<button class="button button-secondary close-ticket"><?php esc_html_e( 'Cancel', 'webappick-pdf-invoice-for-woocommerce' ); ?></button>
				</div>
			</div>
			<?php } ?>
			<div class="wapk-dr-modal-wrap reason">
				<div class="wapk-dr-modal-header">
					<h3><?php esc_html_e( 'If you have a moment, please let us know why you are deactivating:', 'webappick-pdf-invoice-for-woocommerce' ); ?></h3>
					<a href="javascript:void 0;" class="wapk-dr-modal-close" aria-label="<?php esc_attr_e( 'Close', 'webappick-pdf-invoice-for-woocommerce' ); ?>">
						<!--suppress HtmlUnknownAttribute -->
						<svg class="" focusable="false" viewBox="0 0 24 24" aria-hidden="true" role="presentation">
							<path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path>
				...
				<div class="wapk-dr-modal-body">
					...
					<div class="response" style="<?php echo ( $showSupportTicket ) ? 'display: block;' : ''; ?>">
						<div class="wrapper">
						<?php if ( $showSupportTicket ) { ?>
							<p><?php esc_html_e( 'In trouble? Please submit a support request.', 'webappick-pdf-invoice-for-woocommerce' ); ?></p>
							<p>
								<a href="#" class="button button-secondary not-interested"><?php esc_html_e( 'Not Interested', 'webappick-pdf-invoice-for-woocommerce' ); ?></a>
								<button class="button button-primary open-ticket-form"><?php esc_html_e( 'Open Support Ticket', 'webappick-pdf-invoice-for-woocommerce' ); ?></button>
							</p>
						<?php } ?>
						</div>
					</div>
				...
				<div class="wapk-dr-modal-footer">
					<a href="#" class="button button-link dont-bother-me disabled"><?php esc_html_e( 'I rather wouldn\'t say', 'webappick-pdf-invoice-for-woocommerce' ); ?></a>
					<button class="button button-secondary deactivate disabled"><?php esc_html_e( 'Submit & Deactivate', 'webappick-pdf-invoice-for-woocommerce' ); ?></button>
					<button class="button button-primary modal-close disabled"><?php esc_html_e( 'Cancel', 'webappick-pdf-invoice-for-woocommerce' ); ?></button>
				</div>
			</div>
		</div>
					...
					function _ajax(data, buttonElem, cb) {
						...
						return $.ajax({
							...
							beforeSend: function () {
								...
								buttonElem.text('<?php esc_html_e( 'Processing...', 'webappick-pdf-invoice-for-woocommerce' ); ?>');
							},
